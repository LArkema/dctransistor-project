...
sed "s|$OldVer|$NewVer|g" bom.json

echo "After first check" >&2
...
NewVer=$(find ~/ -path "*esp8266*" -name "package.json" | grep -E "esp8266\/[0-9]+.[0-9]+.[0-9]*" -o | cut -d '/' -f 2) #Version is in path name after "esp8266/" and is a series of 2-3 numbers separated by digits (e.g. 3.0.2)
sed -i "s/$OldVer/$NewVer/" bom.json