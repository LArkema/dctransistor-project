...
class SimpleList {
    ...
    int insert(uint8_t station_num){
      ...
      for(int8_t i = 0; i < len; i++){
        if(station_num < stations[i]){
          Serial.printf("Station %d is smaller than station %d. Inserting it at index %d", station_num, stations[i], i);
          if(len-i > 0){
  ...
  public:
    ...
    int setInitialState(uint16_t *train_positions, uint8_t train_len, const uint16_t *station_circuits, uint8_t station_len){
